...
//		sa.assertEquals(getElementText(home.getBrandingElement()), getDataFromExcel("Sheet1", 0, 0));
	...
	public void TC004()
	{
		unhighlight(home.getStudyMaterialElement());
		createTest("TC003=Check whether BYJU's Answer Button is present or not.");
		testLog("Start the Test");
		elementPresentOrNot(home.getByjuAnswerElement());
		highlight(home.getByjuAnswerElement());
		testResult("BYJU's Answer Button is present", "BYJU's Answer Button is not present");
	}

	@Test
	public void TC005()
	{
		createTest("TC005=Check whether BYJU's Answer button is enabled or not.");
		testLog("Start the test.");
		elementEnableOrNot(home.getByjuAnswerElement());
		testResult("BYJU's Answer button is enabled", "BYJU's Answer button is disabled");
	}

	@Test
	public void TC006()
	{
		unhighlight(home.getByjuAnswerElement());
		createTest("TC006=Check whether Scolership Drop down is present or not.");
		testLog("Start the test");
		elementPresentOrNot(home.getScolershipElement());
		testResult("Scolership Drop down is present", "Scolership Drop down is not present");
	}
